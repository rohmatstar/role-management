...
namespace Role_Management
{
    public class UserManager
    {
        ...
        public bool CreateUser(string firstName, string lastName, string username, string password, int role)
        {
            if (ValidateUser(firstName, lastName, password))
            {
                int userID = nextID;
                User newUser = new User(userID, firstName, lastName, username, password, role);
                users.Add(newUser);
                nextID++;
                return true;
            }
        ...
        public void ShowUsers()
        {
            ...
            foreach (User user in users)
            {
                ...
                Console.WriteLine($"Role        : {user.GetUserRole()}");
                Console.WriteLine("========================");
            }
        }
        ...
        public bool ValidateRole(int role)
        {
            if (role != 0 && role != 1)
            {
                return false;
